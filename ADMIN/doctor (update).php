...
<?php
	if(isset($_POST["update"]))
	{
		...
		$q="update doctors set name='".$_POST["name"]."',designation='".$_POST["designation"]."',message='".$_POST["message"]."',pic='".$n."' where id='".$_POST["selectid"]."'";	
		$a=mysqli_query($cn,$q);
		if ($a>0)
		 {
	...
	{
		$cn=mysqli_connect("localhost","root","","medilab");
		$q="delete from doctors where id='".$_POST["selectid"]."'";
		$a=mysqli_query($cn,$q);
		if ($a>0)
		 {
	...
	if (isset($_POST["selectid"]))
	 {
		$cn=mysqli_connect("localhost","root","","medilab");
		$res="select * from doctors where id='".$_POST["selectid"]."'";
	...
	?>
    <    <div class="container">
        <h2>Medical Professional Form</h2>
        ...
        <table>
			<tr>
				<td>
					<select name="selectid" class="select-dropdown" name="selectid" onchange="this.form.submit()">
						...
						<?php
						$cn=mysqli_connect("localhost","root","","medilab");
						$q=mysqli_query($cn,"select * from doctors");
            ...
            <div class="form-group">
                ...
                <input type="text" id="designation" name="designation" value="<?php if(isset($_POST["selectid"])){echo $designation;}?>">
            </div>

            <div class="form-group">
                <label for="message">Message:</label>
                <textarea id="message" name="message"><?php if(isset($_POST["selectid"])){echo $message;}?></textarea>
            </div>

            <label for="image" style="margin-top:10px; margin-bottom:10px;">Image:</label>
            <div class="image-preview">
                <?php if(isset($_POST['selectid']) && !empty($pic)) { ?>
                    <img src="img/<?php echo $pic; ?>" alt="Preview">
                    <?php } 
                    else { ?>
                    No image available
                    <?php } ?>
